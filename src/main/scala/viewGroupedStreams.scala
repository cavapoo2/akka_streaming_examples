...
object ViewGroupedStreams extends App {

  implicit val actorSystem = ActorSystem("View")
  ...
  def processFile(files:List[String]): Unit = {
    Source(file)
      ...
      .groupBy(MaxWords,identity)//.map(println(_)).mergeSubstreams
      .map(_ -> 1)//.map(println(_)).mergeSubstreams  //map to (word,1) tuple
      .reduce((l,r) => (l._1,l._2 + r._2))//.map(println(_)).mergeSubstreams - all tuples with matching words are summed
     .mergeSubstreams
      .runWith(Sink.ignore)
  }
  processFile(file)
